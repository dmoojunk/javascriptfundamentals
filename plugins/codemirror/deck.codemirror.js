  ...
  */
  ...
  var codemirrorify = function(slide) {
    ...
    $.each(codeblocks, function(i, codeblock) {
      ...
      if (!$.data(codeblock, opts.data.codemirrorified) && !codeblock.classList.contains('solution-code')) {
        ...
        if (opts.codemirror.runnable || codeblock.attr("runnable")) {
          ...
          solutionButton.click(function(editor, output){
            return function(event) {
              if($(event.target).parent().next('.solution-code').text()) {
                editor.setValue($(event.target).parent().next('.solution-code').text());
              }
            };
          }(editor, output));
